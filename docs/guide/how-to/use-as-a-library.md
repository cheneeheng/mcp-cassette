...
## Streamable HTTP: URL substitution
...
`MCP_CASSETTE_MODE` > the `mode=` argument > `"once"`. The environment stays the top tier
so the CI invariant holds through this door too: with `MCP_CASSETTE_MODE=none`, a harness
that hard-codes `mode="all"` still cannot record.

| Mode | Behavior |
|---|---|
| `once` (default) | Record if the cassette is absent, replay if it is present. |
| `none` | Replay only; a missing cassette raises `CassetteError`. |
| `all` | Always re-record. |
| `new_episodes` | Replay known interactions; fall misses through to the real server and append them. |
...
## What the block raises, and when
...
## The report sidecar goes to a temp directory
...
- [Record and replay a stdio server](record-and-replay.md)
- [Record and replay a remote HTTP server](remote-http.md)
- [Replay timing](replay-timing.md)
