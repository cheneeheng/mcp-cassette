...
pytest runs in. See [Install](operations/install.md).
...
[Redact secrets](how-to/redact-secrets.md).
...
setup is in [CI pipeline](operations/ci.md).

## You're set up

From here:

- Re-record after the server changes: [Record and replay](how-to/record-and-replay.md).
- Test failure paths without a broken server: [Inject faults](how-to/inject-faults.md).
- Your MCP server is remote, not a local command:
  [Remote HTTP servers](how-to/remote-http.md).
