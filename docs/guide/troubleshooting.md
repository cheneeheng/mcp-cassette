...
[runbook](operations/runbook-replay-misses.md).
...
| `replay had N unmatched request(s)` | The agent asked for something the cassette does not contain, or a param drifted. | See [the runbook](operations/runbook-replay-misses.md#incident-1--replay-had-unmatched-requests); usually `ignore_params` or `new_episodes`. |
| `invalid mcp_cassette mode 'X'; expected one of (...)` | A typo in `MCP_CASSETTE_MODE`, the marker, or the ini option. | Use `once`, `none`, `all`, or `new_episodes`. |
...
| A secret appears in a committed cassette | The value's key matched no redaction rule, or it lives inside a text body. | Rotate the credential, then re-record with a `--redact` JSON-pointer rule. See [Redact secrets](how-to/redact-secrets.md). |

## Still stuck

1. Look at the cassette. It is plain JSON — `mcp-cassette inspect <path>` for the shape,
   an editor for the exact bytes.
